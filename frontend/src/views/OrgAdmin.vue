<template>
  <div class="h-full flex flex-col relative bg-dark min-h-screen">
    ...
    <div class="flex-1 overflow-y-auto pb-24 pt-4 px-4 space-y-6">
      ...
      <div v-if="activeTab === 'equipe'" class="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-8">
        ...
        <div class="space-y-3">
          ...
          <!-- Pending Invites -->
          <h3 v-if="pendingMemberships.length > 0 || activeInvites.length > 0" class="text-[10px] font-bold text-slate-500 uppercase tracking-widest pl-1 mt-6 mb-2">Convites Pendentes / Links Ativos</h3>
          <div v-for="pending in pendingMemberships" :key="pending.id" class="glass p-3 rounded-2xl border border-white/5 border-dashed flex flex-col gap-3 opacity-80">
            <div class="flex items-center justify-between gap-3">
              ...
              <button @click="deletePendingMembership(pending.id, $event)" class="text-rose-400 hover:text-rose-300 p-2">
          ...
          <div v-for="invite in activeInvites" :key="invite.id" class="glass p-3 rounded-2xl border border-white/5 border-dashed flex flex-col gap-3">
            <div class="flex items-center justify-between gap-3">
              ...
              <button @click="deleteInvite(invite.id, $event)" class="text-rose-400 hover:text-rose-300 p-2">
                <i class="pi pi-trash"></i>
              </button>
            </div>
    ...
    <!-- Custom ConfirmPopup for Deletions -->
    <ConfirmPopup group="templating">
        <template #message="slotProps">
            <div class="flex flex-col items-center w-full gap-3 border-b border-white/10 p-3 mb-3 pb-0">
                <i :class="slotProps.message.icon" class="text-4xl text-amber-500"></i>
                <p class="text-sm font-bold text-center text-white">{{ slotProps.message.message }}</p>
            </div>
        </template>
    </ConfirmPopup>
...
const deletePendingMembership = async (id, event) => {
  confirm.require({
    target: event.currentTarget,
    group: 'templating',
    message: 'Tem certeza que deseja cancelar e excluir este convite por e-mail?',
    header: 'Confirmar Exclusão',
    icon: 'pi pi-exclamation-triangle',
    acceptLabel: 'Sim',
    rejectLabel: 'Não',
    rejectClass: 'p-button-text p-button-sm mr-2',
...
const deleteInvite = async (id, event) => {
  confirm.require({
    target: event.currentTarget,
    group: 'templating',
    message: 'Tem certeza que deseja excluir este link de convite?',
    header: 'Confirmar Exclusão',
    icon: 'pi pi-exclamation-triangle',
    acceptLabel: 'Sim',
    rejectLabel: 'Não',
