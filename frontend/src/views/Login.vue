  ...
  <div class="min-h-[calc(100vh-56px)] flex flex-col items-center justify-center p-5">
    ...
    <div class="w-full max-w-sm animate-in fade-in slide-in-from-bottom-6 duration-700">
      <div class="bg-white/5 border border-white/10 rounded-3xl p-6 shadow-2xl shadow-black/40">
        <h2 class="text-xl font-bold text-white mb-6 text-center">Entrar na conta</h2>
      ...
      <p class="mt-6 text-center text-slate-500 text-sm">
        Ainda não tem conta?
        <router-link to="/register" class="text-primary-400 font-bold ml-1">Registrar-se</router-link>
      </p>
    </div>
  </div>
</template>
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (err.response && err.response.status === 401) {
      error.value = 'E-mail ou senha inválidos. Se você ainda não tem conta, peça um convite ao administrador.';
    } else {
      error.value = err.response?.data?.error || 'Ocorreu um erro ao entrar. Tente novamente mais tarde.';
    }
  } finally {
    loading.value = false;
  }
